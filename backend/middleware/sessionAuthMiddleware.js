...
const { validateSession, extractRawToken } = require("../services/identity/sessionService");
...
async function sessionAuthMiddleware(req, res, next) {
  ...
  if (!sessionResult.valid) {
    ...
    return res.status(401).json({
      ...
      code: codeMap[sessionResult.reason] || "NOT_AUTHENTICATED",
    });
  }
  ...
  if (user.status === "suspended" || user.status === "disabled") {
    return res.status(403).json({ error: "Account access denied", code: "ACCOUNT_INACTIVE" });
  }
  ...
  if (session.activeOrganizationId) {
    ...
    if (membership) {
      organization = await Organization.findById(membership.organizationId);
    }
  }
  ...
  req.auth = {
    ...
    // Internal references for route handlers — not to be serialised
    _session: session,
    _user: user,
