...
/**
 * Phase 13.12 — Networking Catalogue Importer
 *
 * Promotes the validated networking staging pack into
 * AIRA's physical knowledge catalogue.
 */
...
function buildNetworkingImportPlan() {
  const physical =
    buildPhysicalIndex();
...
function importNetworkingPack(
  ...
  if (
    ...
    const error =
      new Error(
        'Networking catalogue import refused because conflicts were detected.'
      );
...
function printPlan(
  ...
  console.log(
    'AIRA NETWORKING CATALOGUE IMPORT PLAN'
  );
...
function runCli() {
  ...
  try {
    ...
    const result =
      importNetworkingPack();

    console.log(
      '\nNetworking catalogue import completed.'
    );
