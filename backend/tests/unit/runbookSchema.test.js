...
function validRunbookData(overrides = {}) {
  return {
    ...
    name: 'Pod Restart',
    incidentType: 'pod-crash', // legacy field — still accepted
    lifecycle: RUNBOOK_LIFECYCLE.DRAFT,
    semver: '1.0.0',
    runbookId: 'RB-K8S-POD-RESTART',
    risk: { level: RUNBOOK_RISK_LEVEL.HIGH, reversible: true },
    steps: [
      {
        id: 'check-pod',
        order: 1,
...
describe('Runbook constants', () => {
  ...
  test('LIFECYCLE_VALUES covers all six states', () => {
    ...
    expect(values).toEqual(
      expect.arrayContaining(['DRAFT', 'VALIDATED', 'APPROVED', 'ACTIVE', 'DEPRECATED', 'DISABLED'])
    );
  ...
  test('RUNBOOK_ID_REGEX rejects invalid IDs', () => {
    expect(RUNBOOK_ID_REGEX.test('rb-k8s-pod')).toBe(false); // lowercase
    expect(RUNBOOK_ID_REGEX.test('RB_K8S_POD')).toBe(false); // underscores
    expect(RUNBOOK_ID_REGEX.test('K8S-POD')).toBe(false);    // no RB- prefix
    expect(RUNBOOK_ID_REGEX.test('RB-K8S')).toBe(false);     // only one segment after RB-
  });
  ...
  test('STEP_ID_REGEX rejects invalid step IDs', () => {
    expect(STEP_ID_REGEX.test('Check-Pod')).toBe(false);   // uppercase
    expect(STEP_ID_REGEX.test('-bad-start')).toBe(false);  // starts with hyphen
...
describe('Valid runbook', () => {
  ...
  test('defaults lifecycle to DRAFT', () => {
    const doc = new Runbook({ tenantId: 't1', name: 'X', steps: [{ name: 'S', type: 'kubernetes', action: 'do' }] });
    expect(doc.lifecycle).toBe(RUNBOOK_LIFECYCLE.DRAFT);
  });

  test('accepts all valid lifecycle values', () => {
    for (const lc of Object.values(RUNBOOK_LIFECYCLE)) {
      const doc = new Runbook(validRunbookData({ lifecycle: lc }));
      const err = doc.validateSync();
      expect(err).toBeUndefined();
    }
  });
  ...
  test('accepts valid semver strings', () => {
    for (const v of ['1.0.0', '2.3.14', '1.0.0-alpha.1', '10.0.0+build.1']) {
      const doc = new Runbook(validRunbookData({ semver: v }));
      const err = doc.validateSync();
      expect(err).toBeUndefined();
    }
  });
  ...
  test('accepts all valid step types', () => {
    ...
    for (const type of safeTypes) {
      const doc = new Runbook(validRunbookData({
        steps: [{ id: 'step-a', order: 1, name: 'S', type, action: 'act' }],
      }));
      const err = doc.validateSync();
      expect(err).toBeUndefined();
    }
  ...
  test('accepts all valid failure policies on steps', () => {
    for (const policy of Object.values(RUNBOOK_FAILURE_POLICY)) {
      const doc = new Runbook(validRunbookData({
        steps: [{ id: 's1', order: 1, name: 'S', type: 'kubernetes', action: 'act', failurePolicy: policy }],
      }));
      const err = doc.validateSync();
      expect(err).toBeUndefined();
    }
  ...
  test('accepts parameters with all valid types', () => {
    for (const type of Object.values(RUNBOOK_PARAM_TYPE)) {
      // secret-reference must not have a default
      const param = type === RUNBOOK_PARAM_TYPE.SECRET_REFERENCE
        ? { name: 'secret-param', type }
        : { name: `param-${type}`, type, default: null };
      const doc = new Runbook(validRunbookData({ parameters: [param] }));
      const err = doc.validateSync();
      expect(err).toBeUndefined();
    }
  });
...
describe('Required fields', () => {
  test('rejects runbook without tenantId', () => {
    const doc = new Runbook({ name: 'No Tenant', steps: [{ name: 'S', type: 'kubernetes', action: 'a' }] });
    const err = doc.validateSync();
  ...
  test('rejects runbook without name', () => {
    const doc = new Runbook({ tenantId: 't1', steps: [{ name: 'S', type: 'kubernetes', action: 'a' }] });
    const err = doc.validateSync();
    expect(err).toBeDefined();
  ...
  test('rejects step without name', () => {
    const doc = new Runbook(validRunbookData({
      steps: [{ id: 's1', order: 1, type: 'kubernetes', action: 'act' }],
    }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  ...
  test('rejects step without type', () => {
    const doc = new Runbook(validRunbookData({
      steps: [{ id: 's1', order: 1, name: 'Step', action: 'act' }],
    }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  });
  ...
  test('rejects step without action', () => {
    const doc = new Runbook(validRunbookData({
      steps: [{ id: 's1', order: 1, name: 'Step', type: 'kubernetes' }],
    }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  });
...
describe('Invalid lifecycle', () => {
  test('rejects unknown lifecycle value', () => {
    const doc = new Runbook(validRunbookData({ lifecycle: 'RUNNING' }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  ...
  test('rejects lowercase lifecycle value', () => {
    const doc = new Runbook(validRunbookData({ lifecycle: 'active' }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  });
});
...
describe('Invalid semantic version', () => {
  test('rejects non-semver version strings', () => {
    // Note: '' (empty string) is falsy and treated as "not provided" by the
    // validator (!v → pass), so it is intentionally excluded here.
    const invalid = ['1', '1.0', 'v1.0.0', '1.0.0.0', 'latest'];
    for (const semver of invalid) {
      const doc = new Runbook(validRunbookData({ semver }));
      const err = doc.validateSync();
      expect(err).toBeDefined();
...
describe('Invalid risk', () => {
  ...
  test('rejects lowercase risk level', () => {
    const doc = new Runbook(validRunbookData({ risk: { level: 'high' } }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
  });
});
...
describe('Invalid failure policy', () => {
  test('rejects unknown failurePolicy on step', () => {
    const doc = new Runbook(validRunbookData({
      steps: [{
        id: 's1', order: 1, name: 'S', type: 'kubernetes', action: 'act',
...
describe('Malformed retry policy', () => {
  ...
  test('accepts valid retry configuration', () => {
    const doc = new Runbook(validRunbookData({
      steps: [{
        id: 's1', order: 1, name: 'S', type: 'kubernetes', action: 'act',
        retry: { maxAttempts: 3, delaySeconds: 2, backoffMultiplier: 2.0, maxDelaySeconds: 60 },
      }],
    }));
    const err = doc.validateSync();
    expect(err).toBeUndefined();
...
describe('Duplicate step identifiers', () => {
  ...
  test('validateStepIds returns null for empty array', () => {
    expect(Runbook.validateStepIds([])).toBeNull();
  });
});
      ...
      parameters: [
        {
          ...
          default: 'my-hardcoded-secret', // must be rejected
        },
      ],
    }));
...
describe('Verification structure', () => {
  test('accepts valid verification config', () => {
    const doc = new Runbook(validRunbookData({
      verification: {
        ...
        intervalSeconds: 10,
        checks: [
          { id: 'health-check', type: 'service_healthy', params: { service: 'api' } },
        ],
      },
    }));
    const err = doc.validateSync();
  ...
  test('rejects unknown verification strategy', () => {
    const doc = new Runbook(validRunbookData({
      verification: { strategy: 'MAJORITY' },
    }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
    expect(Object.keys(err.errors).some(k => k.includes('strategy'))).toBe(true);
...
describe('Legacy field backward compatibility', () => {
  ...
  test('legacy shell step type is accepted by schema (migration deferred)', () => {
    // The schema accepts 'shell' for backward compat.
    // Semantic validation (validator phase) will reject it.
    const doc = new Runbook(validRunbookData({
      steps: [{
        id: 's1', order: 1, name: 'Legacy shell', type: 'shell', action: 'some-action',
      }],
    }));
    const err = doc.validateSync();
    expect(err).toBeUndefined();
  ...
  test('numeric version field is preserved alongside semver', () => {
    const doc = new Runbook(validRunbookData({ version: 5, semver: '1.2.3' }));
...
describe('Owner type', () => {
  ...
  test('rejects unknown owner type', () => {
    const doc = new Runbook(validRunbookData({
      owner: { name: 'X', ownerType: 'external' },
    }));
    const err = doc.validateSync();
    expect(err).toBeDefined();
    expect(err.errors['owner.ownerType']).toBeDefined();
  });
});
