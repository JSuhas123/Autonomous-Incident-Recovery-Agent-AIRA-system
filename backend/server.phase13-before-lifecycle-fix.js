...
// secret or connection string is missing â€” preventing silent misconfiguration.
const {
  validateEnvironment,
  inspectEnvironment,
} =
...
// ============================================================================
// PHASE 12.1 â€” AUTHORITATIVE AGENT INTELLIGENCE RUNTIME
// ============================================================================
...
// ============================================================================
// PHASE 11.4 â€” WORKFLOW REPLAY / RECOVERY
// ============================================================================
...
if (
  ...
  if (
    ...
    console.warn(
      `[server] âš ï¸  CORS: missing production origin(s): ${missing.join(", ")}`
    );
  }
}
...
const corsOptions = {
  origin(
    ...
    // Return controlled false â€” do NOT throw, which would produce a 500
    console.warn(
...
// 2. Respond 204 to all OPTIONS requests immediately â€” before any auth middleware
app.use(
  (
    req,
    res,
...
// ============================================================================
// PHASE 11.10 â€” STARTUP / SHUTDOWN ADMISSION GATE
// ============================================================================
...
// Lightweight liveness probe â€” always 200 once Express is up
app.get(
  "/health/live",
  (
    req,
...
app.get(
  ...
  async (
    ...
    try {
      ...
      // ======================================================================
      // PHASE 11.5 â€” DEPENDENCY ISOLATION HEALTH
      // ======================================================================
      ...
      // ======================================================================
      // PHASE 11.6 â€” ADMISSION / LOAD PROTECTION
      // ======================================================================
          ...
          .filter(
            (
              ...
              !flag.enabled
               ...
               // ======================================================================
      // PHASE 11.16 â€” PRODUCTION READINESS
      // ======================================================================
      ...
      const health = {
        ...
        // ====================================================================
        // PHASE 11.5 â€” DEPENDENCY ISOLATION
        // ====================================================================
...
// ============================================================================
// PHASE 11.4 â€” REPLAY RECOVERY STATE
// ============================================================================
...
// ============================================================================
// PHASE 11.10 â€” APPLICATION LIFECYCLE
// ============================================================================
...
async function startServer() {
  ...
  try {
     ...
     * Start HTTP first.
     *
     * This allows orchestration platforms to observe /health/live
     * while the application is still initializing.
     *
     * Operational writes remain blocked by the lifecycle admission
     * gate until AIRA reaches READY.
     */
    ...
    /*
     * initializeServices() owns normal infrastructure initialization,
     * including the authoritative database lifecycle through dbService.
     */
    await initializeServices();


    // ========================================================================
    // PHASE 13 — AUTHORITATIVE POSTGRESQL
    // ========================================================================

    /*
     * PostgreSQL is now mandatory.
     *
     * We deliberately do not execute migrations automatically here.
     * Schema migration remains an explicit deployment operation.
     */
    await initializePostgresFoundation();


    // ========================================================================
    // PHASE 12 — AUTHORITATIVE AGENT INTELLIGENCE
    // ========================================================================
    ...
    /*
     * Startup recovery is the final readiness gate.
     *
     * AIRA cannot enter READY until:
     *
     *   PostgreSQL is connected and healthy
     *   core infrastructure initialized
     *   agent intelligence initialized
     *   durable workflow recovery completed
     */
    ...
    /*
     * Startup failure must clean up every resource that was
     * successfully initialized before the failure occurred.
     */
    try {
      await shutdown(
        "STARTUP_FAILURE",
...
// ============================================================================
// PHASE 11.10 â€” SHUTDOWN HELPERS
// ============================================================================
...
async function initializeServices() {
  ...
  try {
    ...
    console.log(
      queueService
        ?.connected
        ? "[server] âœ“ Queue service connected"
        : "[server] âš  Queue service unavailable"
    );
  } catch (
    error
  ) {
  ...
  try {
    ...
    console.log(
      idempotencyService
        ?.connected
        ? "[server] âœ“ Idempotency service connected"
        : "[server] âš  Idempotency service unavailable"
    );
  } catch (
    error
  ) {
  ...
  try {
    ...
    console.log(
      distributedLockService
        ?.connected
        ? "[server] âœ“ Distributed lock Redis connected"
        : "[server] âš  Distributed lock Redis unavailable"
    );
  } catch (
    error
  ) {
  ...
  // ==========================================================================
  // PHASE 11.3 â€” DURABLE WORKFLOW OUTBOX
  // ==========================================================================
  ...
  try {
    ...
    console.log(
      "[workflow-outbox] âœ“ Durable workflow outbox initialized"
    );
  } catch (
    error
  ) {
  ...
  try {
    ...
    if (
      ...
      console.log(
        "[server] âœ“ Multi-instance coordinator started"
      );
    } else {
      /*
  ...
  try {
    ...
    console.log(
      "[server] âœ“ Monitor scheduler started"
    );
  } catch (
    error
  ) {
  ...
  try {
    if (
      ...
      console.log(
        "[memory-cleanup] PostgreSQL global retention scan disabled — scoped cleanup worker required"
      );
    } else if (
      typeof memoryCleanupJob.start ===
      "function"
    ) {
    ...
    console.log(
      "[server] âœ“ Memory cleanup job started"
    );
  } catch (
    error
  ) {
  ...
  try {
    ...
    console.log(
      "[server] âœ“ Retry processor job started"
    );
  } catch (
    error
  ) {
...
// ============================================================================
// PHASE 11.4 / 11.10 â€” STARTUP REPLAY RECOVERY
// ============================================================================
...
async function runStartupRecovery() {
  ...
  try {
    ...
    console.log(
      `[replay-recovery] âœ“ Startup recovery completed discovered=${replayRecoveryStatus.discovered} recovered=${replayRecoveryStatus.recovered} failed=${replayRecoveryStatus.failed}`
    );
...
async function startServer() {
  ...
  try {
    ...
    // =========================================================================
    // PHASE 13.2 â€” POSTGRESQL FOUNDATION
    // =========================================================================
    ...
    console.log(
      "[server] âœ“ Authoritative agent intelligence runtime initialized"
    );
    ...
    if (
      ...
      console.log(
        `[server] âœ“ AIRA operationally ready on port ${PORT}`
      );
    } else {
      console.warn(
...
// ============================================================================
// PHASE 11.10 â€” GRACEFUL SHUTDOWN
// ============================================================================
